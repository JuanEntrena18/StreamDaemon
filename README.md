...
- **💬 Live Chat** — Twitch IRC chat with real-time forwarding to overlays via Socket.IO. Includes message sending, reply (↩ @user), moderation (timeout/ban), role badges, notification sound selector with volume control, **TTS (text-to-speech)** with voice selection, speed and volume. TTS continues reading messages even when browsing other sections of the app, with configurable content filters (exclude own messages, links, bots) and "read author" mode to include the username before each message.
...
- **📈 KPI Analytics & Twitch Tracker** — Comprehensive channel performance dashboard. Historical stats (7d, 30d, 90d, all time), audience analytics with live viewer snapshots, performance per game with automatic box art fetching, best streaming slots analysis, and **Global Top Viewers** to track the most popular games on Twitch in real-time. Includes an interactive SVG chart of stream evolution and a multi-factor smart advice engine with optional local Ollama AI integration.
...
- **📊 Stream HUD** — Live stats panel (viewers, followers, subs, uptime, game, last follower, last subscriber) with auto-polling, configurable per-field display toggles and dynamic overlay URL with per-field query parameters.
...
- **🖥️ Premium dashboard** — Interface with collapsible sidebar (expands from 56px icons to 260px full navigation), responsive drawer mode on mobile with hamburger menu, glassmorphism, Framer Motion animations, purple/indigo palette, Twitch user badge and real-time connection status. **Auto-scaling UI** with zoom levels (100%, 125%, 150%, 200%) for 2K/4K monitors, configurable from Settings and persisted. All components use **CSS Modules** (`*.module.css`) instead of inline styles for maintainability and performance.
...
- **🌐 Multilingual Interface** — Fully translated into English, Spanish, French, German, and Italian with auto-detection of the browser's language (`navigator.languages`) and persistence.
- **🔍 Global Command Palette** — Press `Ctrl+K` from anywhere in the app to open a global search bar with real-time filtering, fuzzy keyword matching, and keyboard navigation to quickly switch tabs or toggle settings.
- **⚡ High Performance** — UI tabs are dynamically lazy-loaded using `React.lazy()` and `<Suspense>`, significantly splitting the bundle size and reducing the initial startup time in the Electron desktop app.
...
| **Fortnite · BRB** | `http://localhost:3000/overlays/brb_fortnite_sala_de_espera.html?channel=yourchannel` |
| **8-bit · BRB** | `http://localhost:3000/overlays/brb_8_bits_salvapantallas_bouncing.html?channel=yourchannel` |
| **Win95 · BRB** | `http://localhost:3000/overlays/brb_win95_error_screensaver.html?channel=yourchannel` |
| **RetroWave · BRB** | `http://localhost:3000/overlays/brb_retrowave_autopista_vhs.html?channel=yourchannel` |
| **Tactical · BRB** | `http://localhost:3000/overlays/brb_bsg_t_ctico_dradis_sweeping.html?channel=yourchannel` |
| **Horde · BRB** | `http://localhost:3000/overlays/brb_horda.html?channel=yourchannel` |
...
| **Horde Vertical · Start** | `http://localhost:3000/overlays/inicio_vertical_horda.html?channel=yourchannel` |
| **Horde Vertical · End** | `http://localhost:3000/overlays/despedida_vertical_horda.html?channel=yourchannel` |
| **Horde Vertical · BRB** | `http://localhost:3000/overlays/brb_vertical_horda.html?channel=yourchannel` |
...
> **Stream HUD** overlay supports per-field URL parameters to customize which stats are shown: append `&viewers=1&followers=1&subs=1&uptime=0&game=0&title=0&lastFollower=1&lastSub=0` to toggle individual fields. Fields default to the configuration saved in the HUD Panel.
...
│   │   └── public/overlays/   # 57 standalone HTML overlays
│   │       ├── js/
...
│   │       ├── *vertical_horda* # Horde vertical overlays (start, end, brb, alert)
